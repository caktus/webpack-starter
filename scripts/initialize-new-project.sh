...
if [ "$1" != "" ]; then
    ...
    if [ -d "$DIR_TEST" ]; then
        ...
        read -r -p "Are you sure you want to continue? You might lose something... [y/N]" RESPONSE
        case "$RESPONSE" in
            [yY][eE][sS]|[yY])
                ;;
            *)
    ...
    echo ""
    echo ""
    printf "${GREEN}Don't forget to change ${BLUE}\`\"name\": \"webpack-starter\"\` ${GREEN}to ${BLUE}\`\"name\": \"$1\"\` ${GREEN}in package.json!${NOCOLOR} \n"
    echo ""
    echo ""
    exit 0
else
